...
`agent-app-leak`은 **x86-64 Linux 전용 바이너리**다. Apple Silicon Mac(M1/M2/M3/M4)에서는 반드시 `--platform linux/amd64` 옵션으로 Docker를 실행해야 한다.

```bash
# Apple Silicon에서 올바른 Docker 실행 명령
...
`--platform` 없이 실행하면 `rosetta error: failed to open elf at /lib64/ld-linux-x86-64.so.2` 에러가 발생한다.
